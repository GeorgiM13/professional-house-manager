...
BUILDING_ID = 8      # ID за Цар Асен 31
...
for index, row in df_melted.iterrows():
    ...
    # --- 3. ГРУПИ ЗА ДРУГИ (OTHER) ---
        ...
        'батерия', 'материали', 'други', 'ключар'
    ]):
        expense_type = 'other'
...
if PREVIEW_MODE:
    ...
    # ВЪРНАХ ВСИЧКИ КОЛОНИ ТУК:
    header = f"{'TYPE':<25} | {'MO':<2} | {'YEAR':<4} | {'AMOUNT':<8} | {'PAID':<4} | {'BLD':<3} | {'NOTES'}"
    print("-" * len(header))
    print(header)
    print("-" * len(header))

    for item in data_to_upload[:1000]: # Показвам първите 60 реда
